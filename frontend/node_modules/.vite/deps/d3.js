...
function contours_default() {
  ...
  function isorings(values, value, callback) {
    ...
    while (++x2 < dx - 1) {
      t0 = t1, t1 = above(values[x2 + 1], value);
      cases[t0 | t1 << 1].forEach(stitch);
    }
    cases[t1 << 0].forEach(stitch);
    while (++y2 < dy - 1) {
      x2 = -1;
      t1 = above(values[y2 * dx + dx], value);
      t2 = above(values[y2 * dx], value);
      cases[t1 << 1 | t2 << 2].forEach(stitch);
      while (++x2 < dx - 1) {
        t0 = t1, t1 = above(values[y2 * dx + dx + x2 + 1], value);
        t3 = t2, t2 = above(values[y2 * dx + x2 + 1], value);
    ...
    function stitch(line) {
      var start = [line[0][0] + x2, line[0][1] + y2], end = [line[1][0] + x2, line[1][1] + y2], startIndex = index2(start), endIndex = index2(end), f, g;
...
function density_default() {
  var x2 = defaultX, y2 = defaultY, weight = defaultWeight, dx = 960, dy = 500, r = 20, k2 = 2, o = r * 3, n = dx + o * 2 >> k2, m2 = dy + o * 2 >> k2, threshold2 = constant_default3(20);
  function grid(data) {
  ...
  function density(data) {
    ...
    return contours_default().size([n, m2]).thresholds(tz.map((d) => d * pow4k))(values).map((c3, i) => (c3.value = +tz[i], transform2(c3)));
  }
  ...
  density.x = function(_) {
    return arguments.length ? (x2 = typeof _ === "function" ? _ : constant_default3(+_), density) : x2;
  };
  density.y = function(_) {
    return arguments.length ? (y2 = typeof _ === "function" ? _ : constant_default3(+_), density) : y2;
  };
  ...
  *neighbors(i) {
  ...
  _clipSegment(x06, y06, x12, y12, c0, c1) {
    ...
    while (true) {
      ...
      let x2, y2, c3 = c0 || c1;
      if (c3 & 8) x2 = x06 + (x12 - x06) * (this.ymax - y06) / (y12 - y06), y2 = this.ymax;
      else if (c3 & 4) x2 = x06 + (x12 - x06) * (this.ymin - y06) / (y12 - y06), y2 = this.ymin;
      else if (c3 & 2) y2 = y06 + (y12 - y06) * (this.xmax - x06) / (x12 - x06), x2 = this.xmax;
      else y2 = y06 + (y12 - y06) * (this.xmin - x06) / (x12 - x06), x2 = this.xmin;
      if (c0) x06 = x2, y06 = y2, c0 = this._regioncode(x06, y06);
  ...
  _project(x06, y06, vx, vy) {
    let t = Infinity, c3, x2, y2;
    if (vy < 0) {
      if (y06 <= this.ymin) return null;
      if ((c3 = (this.ymin - y06) / vy) < t) y2 = this.ymin, x2 = x06 + (t = c3) * vx;
    } else if (vy > 0) {
      if (y06 >= this.ymax) return null;
      if ((c3 = (this.ymax - y06) / vy) < t) y2 = this.ymax, x2 = x06 + (t = c3) * vx;
    }
    if (vx > 0) {
      if (x06 >= this.xmax) return null;
      if ((c3 = (this.xmax - x06) / vx) < t) x2 = this.xmax, y2 = y06 + (t = c3) * vy;
    } else if (vx < 0) {
      if (x06 <= this.xmin) return null;
      if ((c3 = (this.xmin - x06) / vx) < t) x2 = this.xmin, y2 = y06 + (t = c3) * vy;
    }
    return [x2, y2];
  }
  _edgecode(x2, y2) {
...
var Delaunay = class _Delaunay {
  ...
  *neighbors(i) {
  ...
  find(x2, y2, i = 0) {
    ...
    const i0 = i;
    let c3;
    while ((c3 = this._step(i, x2, y2)) >= 0 && c3 !== i && c3 !== i0) i = c3;
    return c3;
  }
  _step(i, x2, y2) {
...
function centroidLinePoint(lambda, phi2) {
  lambda *= radians, phi2 *= radians;
  var cosPhi = cos2(phi2), x2 = cosPhi * cos2(lambda), y2 = cosPhi * sin2(lambda), z = sin2(phi2), w = atan2(sqrt2((w = y0 * z - z0 * y2) * w + (w = z0 * x2 - x0 * z) * w + (w = x0 * y2 - y0 * x2) * w), x0 * x2 + y0 * y2 + z0 * z);
  W1 += w;
  X1 += w * (x0 + (x0 = x2));
  Y1 += w * (y0 + (y0 = y2));
  Z1 += w * (z0 + (z0 = z));
...
function centroid_default(object2) {
  ...
  if (m2 < epsilon22) {
    x2 = X1, y2 = Y1, z = Z1;
    if (W1 < epsilon6) x2 = X0, y2 = Y0, z = Z0;
    m2 = hypot(x2, y2, z);
    if (m2 < epsilon22) return [NaN, NaN];
  }
  return [atan2(y2, x2) * degrees, asin(z / m2) * degrees];
}
...
function compose_default(a3, b) {
  ...
  if (a3.invert && b.invert) compose.invert = function(x2, y2) {
...
function compareIntersection(a3, b) {
  return ((a3 = a3.x)[0] < 0 ? a3[1] - halfPi2 - epsilon6 : halfPi2 - a3[1]) - ((b = b.x)[0] < 0 ? b[1] - halfPi2 - epsilon6 : halfPi2 - b[1]);
}
...
function graticuleX(y06, y12, dy) {
  var y2 = range(y06, y12 - epsilon6, dy).concat(y12);
  return function(x2) {
...
function graticuleY(x06, x12, dx) {
  var x2 = range(x06, x12 - epsilon6, dx).concat(x12);
  return function(y2) {
...
function interpolate_default(a3, b) {
  var x06 = a3[0] * radians, y06 = a3[1] * radians, x12 = b[0] * radians, y12 = b[1] * radians, cy0 = cos2(y06), sy0 = sin2(y06), cy1 = cos2(y12), sy1 = sin2(y12), kx0 = cy0 * cos2(x06), ky0 = cy0 * sin2(x06), kx1 = cy1 * cos2(x12), ky1 = cy1 * sin2(x12), d = 2 * asin(sqrt2(haversin(y12 - y06) + cy0 * cy1 * haversin(x12 - x06))), k2 = sin2(d);
  var interpolate = d ? function(t) {
    var B2 = sin2(t *= d) / k2, A = sin2(d - t) / k2, x2 = A * kx0 + B2 * kx1, y2 = A * ky0 + B2 * ky1, z = A * sy0 + B2 * sy1;
    return [
      atan2(y2, x2) * degrees,
      atan2(z, sqrt2(x2 * x2 + y2 * y2)) * degrees
    ];
  } : function() {
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b0, c0, x12, y12, lambda12, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a3 = a0 + a1, b = b0 + b1, c3 = c0 + c1, m2 = sqrt2(a3 * a3 + b * b + c3 * c3), phi2 = asin(c3 /= m2), lambda22 = abs3(abs3(c3) - 1) < epsilon6 || abs3(lambda04 - lambda12) < epsilon6 ? (lambda04 + lambda12) / 2 : atan2(b, a3), p = project(lambda22, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x06, dy2 = y2 - y06, dz = dy * dx2 - dx * dy2;
      if (dz * dz / d2 > delta2 || abs3((dx * dx2 + dy * dy2) / d2 - 0.5) > 0.3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
        resampleLineTo(x06, y06, lambda04, a0, b0, c0, x2, y2, lambda22, a3 /= m2, b /= m2, c3, depth, stream);
        stream.point(x2, y2);
        resampleLineTo(x2, y2, lambda22, a3, b, c3, x12, y12, lambda12, a1, b1, c1, depth, stream);
      }
    }
  }
  return function(stream) {
...
function projectionMutator(projectAt) {
  var project, k2 = 150, x2 = 480, y2 = 250, lambda = 0, phi2 = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate, alpha = 0, sx = 1, sy = 1, theta = null, preclip = antimeridian_default, x06 = null, y06, x12, y12, postclip = identity_default2, delta2 = 0.5, projectResample, projectTransform, projectRotateTransform, cache, cacheStream;
...
function azimuthalInvert(angle2) {
  return function(x2, y2) {
    var z = sqrt2(x2 * x2 + y2 * y2), c3 = angle2(z), sc = sin2(c3), cc2 = cos2(c3);
    return [
      atan2(x2 * sc, z * cc2),
      asin(z && y2 * sc / z)
    ];
...
function mercatorProjection(project) {
  ...
  function reclip() {
    var k2 = pi3 * scale2(), t = m2(rotation_default(m2.rotate()).invert([0, 0]));
...
function conicConformalRaw(y06, y12) {
  ...
  function project(x2, y2) {
    if (f > 0) {
      if (y2 < -halfPi2 + epsilon6) y2 = -halfPi2 + epsilon6;
    } else {
      if (y2 > halfPi2 - epsilon6) y2 = halfPi2 - epsilon6;
    }
    var r = f / pow3(tany(y2), n);
    return [r * sin2(n * x2), f - r * cos2(n * x2)];
  }
  project.invert = function(x2, y2) {
    var fy = f - y2, r = sign(n) * sqrt2(x2 * x2 + fy * fy), l = atan2(x2, abs3(fy)) * sign(fy);
    if (fy * n < 0)
      l -= pi3 * sign(x2) * sign(fy);
...
function encloseBasis3(a3, b, c3) {
  var x12 = a3.x, y12 = a3.y, r1 = a3.r, x2 = b.x, y2 = b.y, r2 = b.r, x3 = c3.x, y3 = c3.y, r3 = c3.r, a22 = x12 - x2, a32 = x12 - x3, b2 = y12 - y2, b3 = y12 - y3, c22 = r2 - r1, c32 = r3 - r1, d1 = x12 * x12 + y12 * y12 - r1 * r1, d2 = d1 - x2 * x2 - y2 * y2 + r2 * r2, d3 = d1 - x3 * x3 - y3 * y3 + r3 * r3, ab4 = a32 * b2 - a22 * b3, xa = (b2 * d3 - b3 * d2) / (ab4 * 2) - x12, xb = (b3 * c22 - b2 * c32) / ab4, ya = (a32 * d2 - a22 * d3) / (ab4 * 2) - y12, yb = (a22 * c32 - a32 * c22) / ab4, A = xb * xb + yb * yb - 1, B2 = 2 * (r1 + xa * xb + ya * yb), C = xa * xa + ya * ya - r1 * r1, r = -(Math.abs(A) > 1e-6 ? (B2 + Math.sqrt(B2 * B2 - 4 * A * C)) / (2 * A) : C / B2);
  return {
    x: x12 + xa + xb * r,
    y: y12 + ya + yb * r,
    r
...
function packSiblingsRandom(circles, random) {
  ...
  var a3, b, c3, n, aa2, ca3, i, j, k2, sj, sk;
...
function treemap_default() {
  ...
  treemap.paddingInner = function(x2) {
    return arguments.length ? (paddingInner = typeof x2 === "function" ? x2 : constant_default5(+x2), treemap) : paddingInner;
  };
  ...
  treemap.paddingTop = function(x2) {
    return arguments.length ? (paddingTop = typeof x2 === "function" ? x2 : constant_default5(+x2), treemap) : paddingTop;
  };
  treemap.paddingRight = function(x2) {
    return arguments.length ? (paddingRight = typeof x2 === "function" ? x2 : constant_default5(+x2), treemap) : paddingRight;
  };
  treemap.paddingBottom = function(x2) {
    return arguments.length ? (paddingBottom = typeof x2 === "function" ? x2 : constant_default5(+x2), treemap) : paddingBottom;
  };
  treemap.paddingLeft = function(x2) {
    return arguments.length ? (paddingLeft = typeof x2 === "function" ? x2 : constant_default5(+x2), treemap) : paddingLeft;
  };
  return treemap;
}
...
function centroid_default3(polygon) {
  var i = -1, n = polygon.length, x2 = 0, y2 = 0, a3, b = polygon[n - 1], c3, k2 = 0;
  while (++i < n) {
    a3 = b;
    b = polygon[i];
    k2 += c3 = a3[0] * b[1] - b[0] * a3[1];
    x2 += (a3[0] + b[0]) * c3;
    y2 += (a3[1] + b[1]) * c3;
  }
...
function cross_default(a3, b, c3) {
...
function arc_default() {
  ...
  arc.centroid = function() {
    var r = (+innerRadius.apply(this, arguments) + +outerRadius.apply(this, arguments)) / 2, a3 = (+startAngle.apply(this, arguments) + +endAngle.apply(this, arguments)) / 2 - pi4 / 2;
...
function area_default5(x06, y06, y12) {
  ...
  x06 = typeof x06 === "function" ? x06 : x06 === void 0 ? x : constant_default6(+x06);
  y06 = typeof y06 === "function" ? y06 : y06 === void 0 ? constant_default6(0) : constant_default6(+y06);
  y12 = typeof y12 === "function" ? y12 : y12 === void 0 ? y : constant_default6(+y12);
  function area(data) {
...
function pie_default() {
  var value = identity_default4, sortValues = descending_default, sort2 = null, startAngle = constant_default6(0), endAngle = constant_default6(tau5), padAngle = constant_default6(0);
  function pie(data) {
    var i, n = (data = array_default3(data)).length, j, k2, sum4 = 0, index2 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da2 = Math.min(tau5, Math.max(-tau5, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da2) / n, padAngle.apply(this, arguments)), pa = p * (da2 < 0 ? -1 : 1), v2;
    for (i = 0; i < n; ++i) {
...
function areaRadial_default() {
  var a3 = area_default5().curve(curveRadialLinear), c3 = a3.curve, x06 = a3.lineX0, x12 = a3.lineX1, y06 = a3.lineY0, y12 = a3.lineY1;
...
var wye_default = {
  draw(context, size) {
    ...
    context.lineTo(x12, y12);
    context.lineTo(x2, y2);
    context.lineTo(c2 * x06 - s * y06, s * x06 + c2 * y06);
    context.lineTo(c2 * x12 - s * y12, s * x12 + c2 * y12);
    context.lineTo(c2 * x2 - s * y2, s * x2 + c2 * y2);
    context.lineTo(c2 * x06 + s * y06, c2 * y06 - s * x06);
    context.lineTo(c2 * x12 + s * y12, c2 * y12 - s * x12);

...
from app.rag.generator import expand_query, generate_cited_answer
...
class RAGPipeline:
    ...
    async def run(self, request: QueryRequest, user_id: str | None = None) -> QueryResponse:
        ...
        cache_key = hashlib.md5(
            f"{request.query}:{sorted(request.layer_filter or [])}:{request.include_graph_context}".encode("utf-8")
        ).hexdigest()
        ...
        reranked = await self.reranker.rerank(query=clean_query, chunks=all_chunks, top_n=settings.RERANK_TOP_N)
